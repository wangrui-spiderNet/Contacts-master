 ...
 */
package org.kymjs.contacts.adapter;
...
public class ContactAdapter extends KJAdapter<Contact> implements SectionIndexer {
    ...
    public ContactAdapter(AbsListView view, ArrayList<Contact> mDatas,
                          OnEditAddListListener editCancelListListener) {
        super(view, mDatas, R.layout.item_list_contact);
        datas = mDatas;
        this.editCancelListListener = editCancelListListener;
        if (datas == null) {
            datas = new ArrayList<>();
        }
    ...
    @Override
    public void convert(AdapterHolder holder, Contact item, boolean isScrolling,final int position) {
        ...
        CheckBox chooseStudentCheckbox = holder.getView(R.id.comment_student_checkbox);
        ...
        chooseStudentCheckbox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked){
                    datas.get(position).setSelected(isChecked);
                    editCancelListListener.editAddList(position,true);
                }else{
                    datas.get(position).setSelected(false);
                    editCancelListListener.editAddList(position,false);
                }
        ...
        chooseStudentCheckbox.setChecked(datas.get(position).isSelected());
    }
    ...
    public interface OnEditAddListListener {
        void editAddList(int position, boolean isAdd);
    }
}
